...
export async function initShell(currentPage) {
  ...
  return new Promise(resolve => {
    onAuthStateChanged(auth, async user => {
      ...
      // Update avatar & name
      const initial = (user.displayName || user.email || "M")[0].toUpperCase();
      const avatarEls = document.querySelectorAll(".user-avatar");
      avatarEls.forEach(el => el.textContent = initial);

      const nameEls = document.querySelectorAll(".user-name");
      nameEls.forEach(el => el.textContent = user.displayName || user.email.split("@")[0]);
...
export function updateUsagePill(userData, limits) {
  ...
  const warn = plan === "free" && ((u.chats||0) >= (l.chats||0) - 1 || (u.images||0) >= (l.images||0) - 1);
